...
describe ActiveInteraction::BooleanAttr do
  describe '#prepare(key, value, options = {})' do
    it 'sets `true` to `true`' do
      expect(described_class.prepare(:key, true)).to eq true
    end

    it 'sets `false` to `false`' do
      expect(described_class.prepare(:key, false)).to eq false
    end

    it 'sets "1" to `true`' do
      expect(described_class.prepare(:key, '1')).to eq true
    end

    it 'sets "0" to `false`' do
      expect(described_class.prepare(:key, '0')).to eq false
    end
